...
export function OAuthProvider({ children }: { children: ReactNode }) {
  ...
  const triggerAuth = useCallback(
    (appId: string, scope: string, onSuccess: () => void) => {
      ...
      if (!isRedirect) {
        ...
        const cleanup = () => {
          window.OpenSDK.removeEventListener(window.OpenSDK.Events.OAuth2Message, handleSuccess);
          window.OpenSDK.removeEventListener(window.OpenSDK.Events.AuthError, onError);
        };

        window.OpenSDK.addEventListener(window.OpenSDK.Events.OAuth2Message, handleSuccess);
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      if (data.isAuthorized || !data.appId) {
        ...
        triggerAuth(data.appId, data.scope, checkStatus);
      }
    } catch (err) {
