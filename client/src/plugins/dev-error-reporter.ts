      ...
      server.middlewares.use("/__dev-error", (req, res) => {
        ...
        req.on("end", () => {
          try {
            ...
            if (data.type === "wps-api-error") {
              const lines = [`\n[wps-api-error] ${data.method} ${data.path} → ${data.status}`];
              if (data.wpsCode) lines.push(`  WPS Code: ${data.wpsCode}`);
              if (data.wpsMessage) lines.push(`  Message: ${data.wpsMessage}`);
              if (data.scope) lines.push(`  Scope needed: ${data.scope}`);
              if (data.authorizeUrl) lines.push(`  Authorize URL: ${data.authorizeUrl}`);
